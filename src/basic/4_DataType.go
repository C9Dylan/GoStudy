...
func main() {
	...
	/**
		3.短变量声明: name:=value(参数名必须是一个没有定义过的参数名称)
	 */
	age := 100
	fmt.Print(age)
	fmt.Print("\n")
	...
	const f1 = 9.9e2 // 2.1 * 10 ^ 2
	const f2 = 9.9e-2 // 2.1 / 10 ^ 2
	const f3 = 9.9E2 // 2.1 * 10 ^ 2
	const f4 = 9.9E-2 // 2.1 / 10 ^ 2
	const f5 = 1. // 小数后可省略不写或者干脆不写小数位
	const f6 float64 = 2
	const f7 = .99 // 2.1 / 10 ^ 2
